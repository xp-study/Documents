...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/1.png)
图中表示是程序与对象的可达关系，目前程序的可达对象有对象1-2-3，对象4-7等五个对象。

**第二步** , 开始标记，程序找出它所有可达的对象，并做上标记。如下图所示：
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/2.png)
所以对象1-2-3、对象4-7等五个对象被做上标记。

**第三步** , 标记完了之后，然后开始清除未标记的对象. 结果如下。
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/3.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/4.png)

从上图来看，全部的GC时间都是包裹在STW范围之内的，这样貌似程序暂停的时间过长，影响程序的运行性能。所以Go V1.3 做了简单的优化,将STW的步骤提前, 减少STW暂停的时间范围.如下所示

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/5.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/6.png)

上图所示，我们的程序可抵达的内存对象关系如左图所示，右边的标记表，是用来记录目前每个对象的标记颜色分类。这里面需要注意的是，所谓“程序”，则是一些对象的跟节点集合。所以我们如果将“程序”展开，会得到类似如下的表现形式，如图所示。
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/7.png)

**第二步** , 每次GC回收开始, 会从根节点开始遍历所有对象，把遍历到的对象从白色集合放入“灰色”集合如图所示。

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/8.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/9.png)
这一次遍历是只扫描灰色对象，将灰色对象的第一层遍历可抵达的对象由白色变为灰色，如：对象2、对象7. 而之前的灰色对象1和对象4则会被标记为黑色，同时由灰色标记表移动到黑色标记表中。

**第四步** , 重复**第三步** , 直到灰色中无任何对象，如图所示。
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/10.png)
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/11.jpeg)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/12.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/13.png)

现在如何三色标记过程不启动STW，那么在GC扫描过程中，任意的对象均可能发生读写操作，如图所示，在还没有扫描到对象2的时候，已经标记为黑色的对象4，此时创建指针q，并且指向白色的对象3。
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/14.png)

与此同时灰色的对象2将指针p移除，那么白色的对象3实则就是被挂在了已经扫描完成的黑色的对象4下，如图所示。
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/15.png)
然后我们正常指向三色标记的算法逻辑，将所有灰色的对象标记为黑色，那么对象2和对象7就被标记成了黑色，如图所示。
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/16.png)
那么就执行了三色标记的最后一步，将所有白色对象当做垃圾进行回收，如图所示。
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/17.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/18.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/19.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/20.png)

------

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/21.png)

------

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/22.png)

------

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/23.png)

------

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/24.png)

------

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/25.png)

 但是如果栈不添加,当全部三色标记扫描之后,栈上有可能依然存在白色对象被引用的情况(如上图的对象9). 所以要对栈重新进行三色标记扫描, 但这次为了对象不丢失, 要对本次标记扫描启动STW暂停. 直到栈空间的三色标记结束.

------

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/26.png)

------

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/27.png)

------

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/28.png)

------

 最后将栈和堆空间 扫描剩余的全部 白色节点清除. 这次STW大约的时间在10~100ms间.

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/29.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/30.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/31.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/32.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/33.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/34.jpeg)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/33.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/36.jpeg)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/37.png)
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/38.jpeg)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/39.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/40.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/41.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/42.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/43.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/44.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/45.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/46.png)
...
![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/47.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/48.png)

![img](https://mc.wsh-study.com/mkdocs/Golang内存回收模式全分析/49.png)
