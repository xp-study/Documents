...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/1.png)
...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/2.png)

UDP 网络编程如下，服务端是没有监听这个动作的，只有执行 bind() 系统调用来绑定端口的动作。

![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/3.png)
...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/4.png)
...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/5.png)

运行这两个程序后，通过 netstat 命令可以看到，TCP 和 UDP 是可以同时绑定同一个端口号的。

![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/6.png)
...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/7.png)

接着，运行第二个 TCP 服务进程的时候，就报错了“Address already in use”，如下图：

![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/8.png)

我上面的测试案例是两个 TCP 服务进程同时绑定地址和端口是：0.0.0.0 地址和8888端口，所以才出现的错误。

如果两个 TCP 服务进程绑定的 IP 地址不同，而端口相同的话，也是可以绑定成功的，如下图：

![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/9.png)
...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/10.png)
...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/11.png)
...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/12.png)
...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/13.png)
...
![img](https://mc.wsh-study.com/mkdocs/TCP和UDP可以使用同一个端口吗/14.jpg)
