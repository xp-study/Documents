...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/1.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/2.png)

整体看的视角是这样的：

![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/3.png)
...
![Packet Header](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/4.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/5.png)

> 为什么要这么设计呢？

我们先来看看 TCP 的问题，TCP 在重传报文时的序列号和原始报文的序列号是一样的，也正是由于这个特性，引入了 TCP 重传的歧义问题。

![TCP 重传的歧义问题](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/6.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/7.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/8.png)

每一个 Frame 都有明确的类型，针对类型的不同，功能也不同，自然格式也不同。

我这里只举例 Stream 类型的 Frame 格式，Stream 可以认为就是一条 HTTP 请求，它长这样：

![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/9.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/10.png)
...
![可用窗口耗尽](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/11.png)

接着，当发送方收到对第 `32~36` 字节的 ACK 确认应答后，则 **滑动窗口往右边移动 5 个字节，因为有 5 个字节的数据被应答确认** ，接下来第 `52~56` 字节又变成了可用窗口，那么后续也就可以发送 `52~56` 这 5 个字节的数据了。

![32 ~ 36 字节已确认](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/12.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/13.png)
...
![接收窗口](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/14.jpg)
...
![HTTP/2](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/15.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/16.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/17.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/18.png)

接着，接收方收到了发送方发送过来的数据，有的数据被上层读取了，有的数据丢包了，此时的接收窗口状况如下：

![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/19.png)
...
![接收窗口触发的滑动](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/20.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/21.png)

31、32、34数据包先到达，基于 offset 被优先乱序确认，但 30 数据包没有确认，所以当前已提交的字节偏移量不变，发送方的缓存区不变。

![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/22.png)

30 到达并确认，发送方的缓存区收缩到阈值，接收方发送 MAX_STREAM_DATA Frame（协商缓存大小的特定帧）给发送方，请求增长最大绝对字节偏移量。

![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/23.png)

协商完毕后最大绝对字节偏移量右移，发送方的缓存区变大，同时发送方发现数据包33超时

![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/24.png)

发送方将超时数据包重新编号为 42 继续发送

![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/25.png)
...
![Connection 流量控制](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/26.png)
...
![img](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/27.png)

## QUIC 是如何迁移连接的？

基于 TCP 传输协议的 HTTP 协议，由于是通过四元组（源 IP、源端口、目的 IP、目的端口）确定一条 TCP 连接。

![TCP 四元组](https://mc.wsh-study.com/mkdocs/如何基于UDP协议实现可靠传输/28.png)
