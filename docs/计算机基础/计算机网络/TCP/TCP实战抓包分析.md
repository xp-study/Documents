...
![提纲](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/1.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/2.jpg)

要抓取上面的 ping 命令数据包，首先我们要知道 ping 的数据包是 `icmp` 协议，接着在使用 tcpdump 抓包的时候，就可以指定只抓 icmp 协议的数据包：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/3.jpg)

那么当 tcpdump 抓取到 icmp 数据包后， 输出格式如下：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/4.jpg)

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/5.jpg)
...
![tcpdump 常用选项类](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/6.jpg)
...
![tcpdump 常用过滤表达式类](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/7.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/8.jpg)

接着把 ping.pcap 文件拖到电脑，再用 Wireshark 打开它。打开后，你就可以看到下面这个界面：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/9.jpg)
...
![ping 网络包](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/10.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/11.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/12.jpg)

接着，在终端二执行下面的 curl 命令：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/13.jpg)

最后，回到终端一，按下 Ctrl+C 停止 tcpdump，并把得到的 http.pcap 取出到电脑。

使用 Wireshark 打开 http.pcap 后，你就可以在 Wireshark 中，看到如下的界面：

![HTTP 网络包](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/14.jpg)
...
![TCP 流量图](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/15.jpg)
...
![取消序列号相对值显示](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/16.jpg)

取消后，Seq 显示的就是真实值了：

![TCP 流量图](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/17.jpg)

可见，客户端和服务端的序列号实际上是不同的，序列号是一个随机值。

这其实跟我们书上看到的 TCP 三次握手和四次挥手很类似，作为对比，你通常看到的 TCP 三次握手和四次挥手的流程，基本是这样的：

![TCP 三次握手和四次挥手的流程](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/18.jpg)
...
![实验环境](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/19.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/20.jpg)

其间 tcpdump 抓包的命令如下：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/21.jpg)

过了一会， curl 返回了超时连接的错误：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/22.jpg)

从 `date` 返回的时间，可以发现在超时接近 1 分钟的时间后，curl 返回了错误。

接着，把 tcp_sys_timeout.pcap 文件用 Wireshark 打开分析，显示如下图：

![SYN 超时重传五次](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/23.jpg)
...
![SYN 超时重传两次](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/24.jpg)
...
![SYN 超时重传](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/25.jpg)

### 实验二：TCP 第二次握手 SYN、ACK 丢包

为了模拟客户端收不到服务端第二次握手 SYN、ACK 包，我的做法是在客户端加上防火墙限制，直接粗暴的把来自服务端的数据都丢弃，防火墙的配置如下：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/26.jpg)

接着，在客户端执行 curl 命令：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/27.jpg)

从 `date` 返回的时间前后，可以算出大概 1 分钟后，curl 报错退出了。

客户端在这其间抓取的数据包，用 Wireshark 打开分析，显示的时序图如下：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/28.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/29.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/30.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/31.jpg)

接着，在客户端执行如下 tcpdump 命令：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/32.jpg)

然后，客户端向服务端发起 telnet，因为 telnet 命令是会发起 TCP 连接，所以用此命令做测试：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/33.jpg)

此时，由于服务端收不到第三次握手的 ACK 包，所以一直处于 `SYN_RECV` 状态：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/34.jpg)

而客户端是已完成 TCP 连接建立，处于 `ESTABLISHED` 状态：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/35.jpg)

过了 1 分钟后，观察发现服务端的 TCP 连接不见了：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/36.jpg)

过了 30 分钟，客户端依然还是处于 `ESTABLISHED` 状态：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/37.jpg)

接着，在刚才客户端建立的 telnet 会话，输入 123456 字符，进行发送：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/38.jpg)

持续「好长」一段时间，客户端的 telnet 才断开连接：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/39.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/40.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/41.jpg)
...
![常规 HTTP 请求](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/42.jpg)

在 Linux 3.7 内核版本中，提供了 TCP Fast Open 功能，这个功能可以减少 TCP 连接建立的时延。

![常规 HTTP 请求 与 Fast  Open HTTP 请求](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/43.jpg)
...
![TCP Fast Open 抓包分析](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/44.jpg)
...
![快速重传机制](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/45.jpg)

TCP 重复确认和快速重传的一个案例，用 Wireshark 分析，显示如下：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/46.jpg)
...
![理想状态下的窗口变化](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/47.jpg)
...
![服务端繁忙状态下的窗口变化](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/48.jpg)
...
![窗口大小在收缩](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/49.jpg)

接着，发送方会 **定时发送窗口大小探测报文** ，以便及时知道接收方窗口大小的变化。

以下图 Wireshark 分析图作为例子说明：

![零窗口 与 窗口探测](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/50.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/51.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/52.jpg)
...
![禁用 Nagle 算法 与 启用 Nagle 算法](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/53.jpg)
...
![关闭 Nagle 算法](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/54.jpg)
...
![TCP 延迟确认](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/55.jpg)

延迟等待的时间是在 Linux 内核中定义的，如下图：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/56.jpg)

关键就需要 `HZ` 这个数值大小，HZ 是跟系统的时钟频率有关，每个操作系统都不一样，在我的 Linux 系统中 HZ 大小是 `1000`，如下图：

![img](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/57.jpg)
...
![关闭 TCP 延迟确认](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/58.jpg)

> 延迟确认 和 Nagle 算法混合使用时，会产生新的问题

当 TCP 延迟确认 和 Nagle 算法混合使用时，会导致时耗增长，如下图：

![TCP 延迟确认 和 Nagle 算法混合使用](https://mc.wsh-study.com/mkdocs/TCP实战抓包分析/59.jpg)
