...
![本文提纲](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/1.jpg)
...
![半连接队列与全连接队列](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/2.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/3.jpg)

在「LISTEN 状态」时，`Recv-Q/Send-Q` 表示的含义如下：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/4.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/5.jpg)
...
![测试环境](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/6.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/7.jpg)

在服务端可以使用 `ss` 命令，来查看当前 TCP 全连接队列的情况：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/8.jpg)

其间共执行了两次 ss 命令，从上面的输出结果，可以发现当前 TCP 全连接队列上升到了 129 大小，超过了最大 TCP 全连接队列。

 **当超过了 TCP 最大全连接队列，服务端则会丢掉后续进来的 TCP 连接** ，丢掉的 TCP 连接的个数会被统计起来，我们可以使用 netstat -s 命令来查看：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/9.jpg)
...
![全连接队列溢出](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/10.jpg)

> Linux 有个参数可以指定当 TCP 全连接队列满了会使用什么策略来回应客户端。

实际上，丢弃连接只是 Linux 的默认行为，我们还可以选择向客户端发送 RST 复位报文，告诉客户端连接已经建立失败。

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/11.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/12.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/13.jpg)

现在我们重新压测，把 TCP 全连接队列 **搞大** ，把 `somaxconn` 设置成 5000：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/14.jpg)

接着把 Nginx 的 backlog 也同样设置成 5000：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/15.jpg)

最后要重启 Nginx 服务，因为只有重新调用 `listen()` 函数 TCP 全连接队列才会重新初始化。

重启完后 Nginx 服务后，服务端执行 ss 命令，查看 TCP 全连接队列大小：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/16.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/17.jpg)

服务端执行 `ss` 命令，查看 TCP 全连接队列使用情况：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/18.jpg)

从上面的执行结果，可以发现全连接队列使用增长的很快，但是一直都没有超过最大值，所以就不会溢出，那么 `netstat -s` 就不会有 TCP 全连接队列溢出个数的显示：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/19.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/20.jpg)
...
![测试环境](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/21.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/22.jpg)

当服务端受到 SYN 攻击后，连接服务端 ssh 就会断开了，无法再连上。只能在服务端主机上执行查看当前 TCP 半连接队列大小：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/23.jpg)

同时，还可以通过 netstat -s 观察半连接队列溢出的情况：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/24.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/25.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/26.jpg)

从源码中，我可以得出共有三个条件因队列长度的关系而被丢弃的：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/27.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/28.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/29.jpg)

从上面的代码中，我们可以算出 max_qlen_log 是 8，于是代入到 检测半连接队列是否满的函数 reqsk_queue_is_full ：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/30.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/31.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/32.jpg)

配置完后，服务端要重启 Nginx，因为全连接队列最大值和半连接队列最大值是在 listen() 函数初始化。

根据前面的源码分析，我们可以计算出半连接队列 max_qlen_log 的最大值为 256：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/33.jpg)

客户端执行 hping3 发起 SYN 攻击：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/34.jpg)

服务端执行如下命令，查看处于 SYN_RECV 状态的最大个数：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/35.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/36.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/37.jpg)
...
![开启 syncookies 功能](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/38.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/39.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/40.jpg)

增大 backlog 的方式，每个 Web 服务都不同，比如 Nginx 增大 backlog 的方法如下：

![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/41.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/42.jpg)
...
![img](https://mc.wsh-study.com/mkdocs/TCP半连接队列和全连接队列/43.jpg)
