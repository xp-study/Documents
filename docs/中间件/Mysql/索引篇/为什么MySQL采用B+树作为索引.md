...
![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/1.png)
...
![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/2.png)
...
![请添加图片描述](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/3.webp)
...
![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/4.webp)

另外，二叉查找树解决了插入新节点的问题，因为二叉查找树是一个跳跃结构，不必连续排列。这样在插入的时候，新节点可以放在任何位置，不会像线性结构那样插入一个元素，所有元素都需要向后排列。

下面是二叉查找树插入某个节点的动图演示：

![请添加图片描述](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/5.webp)
...
![请添加图片描述](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/6.webp)
...
![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/7.webp)
...
![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/8.webp)

 **不管平衡二叉查找树还是红黑树，都会随着插入的元素增多，而导致树的高度变高，这就意味着磁盘 I/O 操作次数多，会影响整体数据查询的效率** 。

比如，下面这个平衡二叉查找树的高度为 5，那么在访问最底部的节点时，就需要磁盘 5 次 I/O 操作。

![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/9.png)

根本原因是因为它们都是二叉树，也就是每个节点只能保存 2 个子节点 ，如果我们把二叉树改成 M 叉树（M>2）呢？

比如，当 M=3 时，在同样的节点个数情况下，三叉树比二叉树的树高要矮。

![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/10.png)
...
![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/11.webp)

我们来看看一棵 3 阶的 B 树的查询过程是怎样的？

![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/12.webp)
...
![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/13.png)
...
![请添加图片描述](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/14.webp)
...
![请添加图片描述](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/15.webp)

甚至，B+ 树在删除根节点的时候，由于存在冗余的节点，所以不会发生复杂的树的变形，比如下面这个动图是删除 B+ 树根节点的过程：

![请添加图片描述](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/16.webp)

B 树则不同，B 树没有冗余节点，删除节点的时候非常复杂，比如删除根节点中的数据，可能涉及复杂的树的变形，比如下面这个动图是删除 B 树根节点的过程：

![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/17.webp)
...
![图片](https://mc.wsh-study.com/mkdocs/为什么MySQL采用B+树作为索引/18.png)
