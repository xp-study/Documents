...
![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/1.jpg)
...
![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/2.jpg)
...
![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/6.jpg)
...
![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/15.jpg)
...
![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/16.jpg)

主线程调用 main 方法，于是为该方法生成一个 main 栈帧：

![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/17.jpg)

那么这个参数 args 的值从哪里来呢？没错，就是从堆中 new 出来的：

![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/18.jpg)
...
![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/19.jpg)

OK，CPU 根据程序计数器的指示，进入 method1 方法，自然，method1 栈帧就被创建出来了：

![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/20.jpg)

局部变量表和方法返回地址安顿好后，就可以开始具体的方法调用了，首先 10 会被传给 x，然后走到 y 被赋值成 x + 1 这步，也就是程序计数器会被修改成这步代码对应的字节码指令的地址：

![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/21.jpg)

走到 `Object m = method2();` 这一步的时候，又会创建一个 method2 栈帧：

![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/22.jpg)

可以看到，method2 方法的第一行代码会在堆中创建一个 Object 对象：

![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/23.jpg)

随后，走到 method2 方法中的 `return n;` 语句，n 指向的堆中的地址就会被返回给 method1 中的 m，并且，满足栈后进先出的原则，method2 栈帧会从虚拟机栈内存中被销毁：

![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/24.jpg)根据 method2 栈帧指向的方法返回地址，我们接着执行 `System.out.println(m.toString())` 这条输出语句，执行完后，method1 栈帧也被销毁了：

![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/25.jpg)
...
![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/26.jpg)

然后以 DEBUG 方式运行，你就会发现存在两个互不干涉的虚拟机栈空间：

![图片](./images/30张图,以DEBUG方式深入理解线程的底层运行原理/27.jpg)
