...
今天和大家聊聊关于 Java 的垃圾回收机制，是 Java 体系中非常重要的一块内容，也是面试必问的内容。

垃圾回收，首先我们要知道哪些是垃圾。。就像电影 *coco* 中所说，一个人的消失是直到这个世界上没有人记着他了，那么内存中的垃圾就是没有被使用的那些对象，也就是没人记着它们了。

所谓使用中的对象，指的是程序中还有引用的对象；而未使用中的对象，指的是程序中已经没有引用的对象，该对象占用的内存也可以被回收掉。

Java 语言出来之前，大家都在写 C 或者 C++ 的程序，此时存在一个很大的矛盾，C++ 等语言创建对象需要不断的去开辟空间，不用的时候又需要不断的去释放空间，既要写构造函数，又要写析构函数，很多时候都在重复的 allocated，然后不停的析构。

而 Java 不一样，它有垃圾回收器，释放内存由回收器负责。

垃圾回收的第一步是标记。垃圾回收器此时会找出内存哪些在使用中，哪些不是。

![图片](./images/一文搞懂Java垃圾回收机制/1.jpg)

上图中，蓝色表示已引用对象，橙色表示未引用对象。垃圾回收器要检查完所有的对象，才能知道哪些有被引用，哪些没。如果系统里所有的对象都要检查，那这一步可能会相当耗时间。

垃圾回收的第二步是清除，这一步会删掉标记出的未引用对象。

![图片](./images/一文搞懂Java垃圾回收机制/2.jpg)

内存分配器会保留指向可用内存中的引用，以分配给新的对象。

垃圾回收的第三步是压缩，为了提升性能，删除了未引用对象后，还可以将剩下的已引用对象放在一起（压缩），这样就能更简单快捷地分配新对象了。

![图片](./images/一文搞懂Java垃圾回收机制/3.jpg)

之前提到过，逐一标记和压缩  Java 虚拟机中的所有对象非常低效：分配的对象越多，垃圾回收需要的时间就越久。不过，根据统计，大部分的对象，其实用没多久就不用了。

来看个例子吧。

下图中，竖轴代表已分配的字节，而横轴代表程序的运行时间。

![图片](./images/一文搞懂Java垃圾回收机制/4.jpg)

可见，存活（没被释放）的对象随着运行时间越来越少。图中左侧的峰值，也表明了大部分对象其实都挺短命的。

那 JVM 是如何提升垃圾回收的效率呢？

根据之前的规律，就可以用来提升 JVM 的效率了。方法是，把堆分成几个部分（就是所谓的分代），分别是新生代、老年代，以及永久代（**JDK8 的时候已经被彻底移除，取而代之的是元空间**）。

![图片](./images/一文搞懂Java垃圾回收机制/5.jpg)
...
![图片](./images/一文搞懂Java垃圾回收机制/6.jpg)

当 eden 空间填满时，会触发轻微的垃圾收集。

![图片](./images/一文搞懂Java垃圾回收机制/7.jpg)

引用的对象被移动到第一个 survivor 空间。清除 eden 空间时，将删除未引用的对象。

![图片](./images/一文搞懂Java垃圾回收机制/8.jpg)
