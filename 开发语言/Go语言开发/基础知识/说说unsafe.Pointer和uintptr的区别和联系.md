...
前几天 Go语言中文网 公众号发了一篇文章：[unsafe 真就不安全吗？](http://mp.weixin.qq.com/s?__biz=MzAxMTA4Njc0OQ==&mid=2651447678&idx=2&sn=5fbee56504c44a9d8c651cc751a650f4&chksm=80bb3f8cb7ccb69a1a2da6596f33d50e6d13311aa5ada7fe03ad94f9a681d632ecd4e29ebc24&scene=21#wechat_redirect)其中谈到了 unsafe 包的初衷和功能。不过那篇文章还有一个功能没讲，那就是 unsafe.Pointer，今天就详细介绍它，包括其他相关类型和实际使用情况。同时还会设计到社区对 unsafe 的看法以及建议。希望看完这篇文章，你能较好的掌握 unsafe.Pointer。当然，请在必要时使用它。
...
Go 有**兼容性指南**[3]，保证版本更新的兼容性。简单地说，它保证你的代码在升级后仍然可以工作，但是不能保证你已经导入了 unsafe 的包。unsafe 包的使用可能会破坏你的代码的每个版本: major，minor，甚至安全修补程序。所以在导入之前，试着想一下这样一种情况：你的客户问你为什么我们不能通过升级 Go 版本来消除漏洞，或者为什么在更新之后什么都不能工作了。
...
我想知道：如果这个包如此危险，有多少冒险者在使用它。我已经在 **GitHub**[4] 上搜索过了。与 **crypto**[5] 或 **math**[6] 相比，数量并不多。其中超过一半的内容是关于使用 unsafe 的方法的技巧和可能的偏差，而不是一些真正的用法。

Rust 社区有一个事件：一个叫 Nikolay Kim 的，他是 **activex**[7] 项目的创始人，在社区的巨大压力下，将 activex 库变成了私有。后来再公开该仓库时，将其中一个贡献者提升为所有者，然后**离开**[8]。所有这一切的发生都是因为一些人认为使用了 unsafe 包，这太危险不应该使用。我知道 Go 社区目前没有这种情况，而且 Go 社区里也没有唯一正确的观点。我想要提醒的是，如果你在代码中导入了 unsafe 的代码，请做好准备，社区可能会。。。

## 爱好者

有很多人和很多想法，**这篇文章**[9]展示了使用 int 和使用指针操作的新方法，简而言之，它看起来像这样：
